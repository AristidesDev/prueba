...
class GetCategorieBaseSpider(scrapy.Spider):
    name = "get_categories_base_primary"
    ...
    def parse(self, response):
        categories_base = response.xpath('.//div[contains(@class, "categories__container")]') # Ajusta el XPath según la estructura real de la página
        for category_base in categories_base:
            ...
            primary = []
            for category_primary in categories_base:
                name_category_primary = category_base.xpath('.//li[contains(@class, "categories__item")]/a/h3/text()').get()
                url_category_primary = category_primary.xpath('.//li[contains(@class, "categories__item")]/a/@href').get()
